...
while IFS= read -r file; do
  ...
  if [[ "${file}" == src/TradingBot.FuturesWorker/* || "${file}" == src/TradingBot.Core/* ]]; then
    worker_changed=true
  fi
...
if [[ "${worker_changed}" == "true" && "${changelog_changed}" != "true" ]]; then
  echo "ERROR: src/TradingBot.FuturesWorker or src/TradingBot.Core changed, but .ai/worker-changelog.md was not updated."
