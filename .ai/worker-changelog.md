...
- `dashboard.today.trades` close rows now expose the same stable `positionId` / `entryTradeId` and an embedded opening snapshot, so a close today retains its real opening context even when the entry was yesterday. Active dashboard positions expose `entryTradeId`; equity exposes `worstDay`, and summary exposes server-calculated `dataAgeSeconds`. Exit aliases, hold duration, fee total, and the source/timestamp of any persisted exit-side signal snapshot are additive fields only.
...
- `Standard` joins `Continuation` and `Reclaim` in the arm's disabled LONG channels. This targets the live eight-day attribution in which Standard LONG produced 1 winner from 18 closes and -30.9521 USD gross; it does not create signals or disable scanning. Breakout LONG and DipBounce remain eligible after the stronger tape/hold confirmation, and SHORT remains enabled with stronger downward tape confirmation.
- Not changed: Lukas configuration, 1x leverage and 30 USD notional on BYKO, position/risk caps, scoring thresholds, spread/depth/funding gates, TP/SL, ATR trailing, execution, reconciliation, and dead-man behavior. Rollback is the five BYKO configuration values plus removing `Standard` from `DisabledLongEntryChannels`.
...
- Why, and it is turnover rather than trade quality: over 2026-08-31..09-01 the arm took 42 entries against the control's 9 and closed 39 against 6. Per close the arm was BETTER (-0.48% vs the control's -0.67%), yet it lost ~35 USD more, because ~12 USD of that gap is pure execution cost (42 x ~0.33 USD of fee plus spread against 9 x ~0.25). With no entry edge - the finding that has held through every cut this session - P&L is approximately minus cost times trade count, so the instance that trades less loses less. Halving the slots halves the cost and the correlated-drawdown exposure (on 08-30 every long bottomed within the same ten minutes).
- Not a parameter fit: the change follows from cost arithmetic, not from picking the best value on a small sample. The D exit itself is untouched - a separate forensic audit of all 28 losing closes showed the tight ATR stop prevented liquidation on ARB (-82.72 USD unrealised at its worst against 20 USD of margin) and CRV (-46.49), so exits stay as they are.
- `LiveTradingEnabled` deliberately NOT changed (it is false in the file and set from the PROD environment anyway); the worker was stopped manually while this was prepared and resumes trading on this deploy. Guard test: `expectedSlots` for the arm is now 3 and the pinned `MaxNotionalUsd` is 300; the control is untouched at 3 slots / 15 USD / 150 notional.

## 2026-09-01-night-blackout-off-on-the-arm

- Owner-directed: `ExecutionPolicy.EntryBlackoutMinutes` 360 -> 0 on futures-live (BYKO), so the 22:00-04:00 UTC entry blackout no longer applies to the arm. The control (futures-lukas-live) keeps it at 360. Rollback = set it back to 360; the window start hour is untouched, and `IsInEntryBlackout` returns false as soon as the minutes are <= 0, so nothing else changes.
- Why, and it is NOT the P&L: the rule exists to avoid thin overnight liquidity, and that premise is contradicted by the data. Median entry spread by hour across the D window is LOWEST at 01-04 UTC (0.175-0.212%) and highest at 21 UTC (0.340%) and 07-09 UTC (0.270-0.283%); p95 in the blackout hours (0.83-1.02%) is below the 07/11/19/20 UTC peaks (1.08-1.15%). Nights are not worse on spread - in places they are better. Depth cannot be checked at all: `market_orderbooks` is keyed (venue,pair) and overwritten, so no historical depth exists.
- Supporting but weak: a point-in-time audit of production decision facts found 5 candidates that passed every other gate and were rejected purely by the blackout (XBT short, TRX short, OPEN long, NEAR long, OP long). Replayed on the real 1m path under the current D exit they net +$2.43, including the window's best trade (OP/USD +$5.36). N=5 over 1.6 days - an early signal, not evidence; the falsified premise is the actual reason for the change.
- Explicitly NOT changed: the 4h stop-loss cooldown (audit: 5 blocked entries, all 5 would have lost, saved ~$6.05 - keep), the 30m close cooldown (audit: 0 rejects in the window - no evidence either way), and the D stop itself. The long-side stop question (grid showed SL 2% with +2% activation better on LONG, -13.16 vs -21.31) is deliberately left alone: N=22 from one episode is exactly the sample size that has failed out-of-sample five times in this project, and a per-side stop would need a new parameter since `StopAtrMult` is global and the sizer never sees the side.
- Guard test: `ExecutionPolicy.EntryBlackoutMinutes` added to the sanctioned arm/control divergence list.
...
- Defect 1 - the placed stop ignored the ATR plan. `FuturesVirtualPortfolio.Open` and `ImportedTpSl` both set the working stop from `TpSl.StopLossPercent` (1.75%), never from `entryPlan.StopDistancePct` (StopAtrMult x ATR). The sizer's ATR stop only shaped notional, never the protective order. Now, under the regime, `Open` places the plan's ATR stop (fixed stop still stands off the regime / when the plan has none).
- Defect 2 - reconcile resurrected the fixed TP. `ImportedTpSl` rebuilt TP/SL from the fixed percents every Kraken sync, re-arming a 3.5% TP on a position the open path had given none - which is how FET got its TP back within four minutes. Now, under the regime, reconcile keeps the position's ATR stop and forces every TP field null; a stray exchange TP order left from before is cancelled once in `EnsureExchangeProtectionOrdersAsync` (`futures-regime-tp-cancel`), not carried.
- Defect 3 - the trail could never arm. `AtrPct` was never set on the position at open (only `EntryAtr` was), and `TryArmRegimeTrailAsync` reads `AtrPct`, so the +1R trail was dead. `Open` now sets `AtrPct = entryPlan.AtrPct`.
- Point that mattered most (owner's): +1R activation is measured against `held.StopDistancePct`, which is now the real ATR stop. So if the ATR stop is 0.6%, activation is +0.6% (not the old +1.75%), and the 1.5x ATR trail follows. No hidden second desync between stop and activation.
- Worked example, entry 100, plan StopDistancePct 0.6%: SL = 99.40, no fixed TP, +1R trail activation at 100.60, then trail = 1.5x ATR. Pinned by tests: open path (ATR stop + no TP + AtrPct carried + 100.60 activation), reconcile (keeps ATR stop, never resurrects TP), and the legacy/off path (fixed 1.75/3.5 stop+TP unchanged, ATR plan ignored). Full suite 279 green.

## 2026-08-30-luko-divergence-marker

- Exit regime D slice 6/7 - the ⚠️ "not like LUKO" marker, and the plan (src/PLAN-exit-regime-D.md) is now deleted. Owner's rule: mark a trade LUKO would NOT have opened or closed as a DECISION, not the TP/SL parameter differences (those differ on every arm trade and would make the flag noise). Today that is a Reversal-book entry (LUKO has no reversal) or a max-hold release close (LUKO has no max-hold). Both are dormant under the current arm config, so the flag stays off until such a rule is turned on - forward-looking by design.
- Derived from data already present (the strategy tag and the close reason / exit mode), so NO new persisted state, DB column or API field. `FuturesEntryAnnouncement.DivergesFromLuko(strategy, reasonCode)` drives a ⚠️ inserted into the Telegram head right after the emoji marks and before the label, on both the open and the close post. zurnalas.html renders a "⚠️ ne kaip LUKO" badge on any open position or closed trade whose tradeStrategy is Reversal or whose exit is a max-hold release. Tests pin the composer marker on/off.
- With the current config (Reversal off, max-hold off, and BYKO's entry filters a strict subset of LUKO's) the marker correctly never fires: BYKO makes the same open/close decisions as LUKO, differing only in size and the exit parameters the owner chose to exclude.
...
- Exits turned LUKO-style on the arm: SignalReversalExitEnabled true (was false) and MaxHoldTrailingStopPercent 0 (was 0.5, off). Measured 2026-08-30 on the arm's own 45-day entries, price exits held constant (stop 1.75 / arm 3.5 / trail 0.5), varying only these two rules, clustered by coin-day: P1 current (max-hold, no signal-exit) -0.121%/coin-day; P2 drop max-hold -0.047%; P3 signal-exit + no max-hold +0.042% (the ONLY policy positive, and positive in BOTH halves); P4 both -0.023%. Max-hold cut winners (it evicted at 6h right before the move, exactly the OP case), and holding until the signal fades was the single biggest gain. Both together are worse than signal-exit alone, so the arm runs signal-exit WITHOUT max-hold.
- Still near-zero in absolute terms (+0.04%/coin-day) - it lifts the exits out of the red to about breakeven, it does not manufacture an entry edge. Control untouched.

## 2026-08-29-preserve-position-state-on-reconcile-and-gate-momentum-shorts-on-a-falling-btc

- ROOT-CAUSE FIX. `ReconcileWithKrakenAsync` rebuilds every open position from Kraken data each cycle, and it was dropping the four fields Kraken cannot tell us: `ExitMode`, `PeakPnlPercent`, `PeakPnlAtUtc`, `Strategy`. Consequences seen live today: a six-hour max-hold release closed with the profit-trail wording (the `MAX_HOLD_TRAIL` marker was blanked, so `ClosureReason` fell back to `EXCHANGE_TRAILING_STOP`); the recorded peak reset to the current tick every cycle (so DOGE showed peak = current and the max-hold 6h clock never saw a real peak); and the channel post lost its `· Momentum` tag. Now carried from `existing`. NOTE: today's deploys restarted the worker mid-position, which is what surfaced this - the reconcile ran and clobbered state that had been correct.
- NEW momentum-short gate, owner's rule. `Shorts.RequireBtc4hDropPercent` (0 = off, control; arm = 0.4): a momentum short is admitted only when BTC has fallen at least that many percent over the last four hours (closed 15m candles, open-of-oldest vs close-of-newest over 16 bars). Flat, slow-drift, or rising BTC blocks it; an unreadable BTC blocks it (the drop is unconfirmed). A HARD gate placed ABOVE the score override in `EvaluateShortGate`, so a high score cannot buy in. The Reversal book never reaches this gate - it fades a sharp RISE on purpose and skips the whole momentum-direction chain - so this does not touch reversal shorts. `CalculateClosedCandle24hChangePct` generalised to `CalculateClosedCandleChangePct(candles, tf, windowMinutes)`; `BtcRegimeState.Change4hPct` added and logged in the regime line.
- Dashboard (`public/zurnalas.html`, not under the changelog rule but shipped here): the armed-trail narrative now says the honest max-hold wording when `exitMode == MAX_HOLD_TRAIL` instead of "went far enough into profit"; and the profit-scale bar's ceiling label no longer carries a money value - it read as an achieved "+22.49 $" peak when it is only the -15% axis edge (identical on every position of the same notional), now shown as "(skalės riba -15%)".
- No change to the control (both new knobs default off / absent on lukas), and no change to scoring, sizing, or the existing exit distances.
...
- `FuturesReversalStrategy.Evaluate` is the whole signal: the move over `Reversal.TriggerWindowMinutes` (whole closed candles) at or beyond `MinMovePercent` fires the fade; beyond `MaxMovePercent` it is read as news/delisting and NOT faded. Thresholds live in config (`Reversal` section), defaults 15 min / 3% / 15% from the 45-day onset study; binder default is DISABLED so the control cannot grow a second book by upgrading. Arm-only: `appsettings.json` enables it, `appsettings.lukas.json` has no section, and the profile guard pins both.
- The reversal entry keeps every protection that guards MONEY: the spread ceiling, the portfolio guards (pending orders, blackout, cooldowns, correlation caps, slots) and the full margin risk layer (ATR, stop cap, funding, volume, depth, leverage, liquidation distance, concurrent-risk cap) via `BuildReversalRiskInputs`, which holds open only the two momentum-DIRECTION gates (BTC regime, short score). Momentum keeps absolute priority: reversal is consulted only when momentum wants nothing on the pair, and slots are ranked by momentum score, so a reversal candidate takes a slot only after momentum candidates that cycle.
- Every trade now carries `Strategy = Momentum | Reversal` end to end: `PortfolioPosition.Strategy` (survives Clone and the state round trip; new `strategy` column on `portfolio_position_state`, appended at the END of the ordinal-read SELECT) and `DryRunAction.Strategy` on opens AND closes (new `strategy` column on `dry_run_actions` with its first `alter table` migration). Momentum entries are tagged at the same site that stamps the entry channel; the close copies the tag from the position. Spot and legacy rows stay null = Momentum. The four books - Momentum/Reversal x LONG/SHORT - are separable in SQL via `strategy` + `side`.
- No momentum selection/entry logic changed; exits are shared (the arm's 3/1.5/0.25 applies to both books). Deliberately NOT done, per the spec: no reversal-specific exits, no grid searches, no scorer.
