...
- The sheet is A4 and a phone is not, so it opened wider than the screen and the first thing a reader did was scroll sideways. It is scaled to the screen width on open, by `zoom` rather than a transform: zoom reflows, where a transform would shrink the sheet and leave its original height behind as empty space. The download button keeps its own size, so it stays readable at any scale.
