...
- One page per theme is kept alive with its fonts already parsed, and only the markup is swapped per request. Rebuilding the document each time meant re-reading a megabyte of embedded TrueType for every crawler, which on the server was 2.6 s per card and none of it was drawing; it is 0.12 s now, API call included. Every variant was rendered through one reused page - result, loss, empty history, and back - to prove nothing sticks between them.
