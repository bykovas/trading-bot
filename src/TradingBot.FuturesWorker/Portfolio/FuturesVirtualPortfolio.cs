    ...
    IClock? clock = null)
{
        ...
        bool flippedEntry)
    {
        ...
        state.Positions.Add(new PortfolioPosition
        {
            ...
            FundingPaidEur = 0m,
            // Exit regime D places NO fixed take-profit: the position leaves only by stop,
            // the ATR trail, or the signal-reversal exit. The stop-loss is untouched. Gating
            // ExchangeTakeProfitPrice to null also stops the live path arming a TP order,
            // since EnsureExchangeProtectionOrdersAsync only places one when it is > 0.
            StopLossPrice = config.TpSl.Enabled ? (side == "SHORT" ? fillPrice + slDistance : fillPrice - slDistance) : null,
            TakeProfitPrice = config.TpSl.Enabled && !config.Exits.AtrTrailingRegimeEnabled ? (side == "SHORT" ? fillPrice - tpDistance : fillPrice + tpDistance) : null,
            StopDistancePct = config.TpSl.Enabled ? slDistancePct : null,
            TakeProfitDistancePct = config.TpSl.Enabled && !config.Exits.AtrTrailingRegimeEnabled ? tpDistancePct : null,
            ExchangeStopLossPrice = config.TpSl.Enabled ? (side == "SHORT" ? fillPrice + exchangeSlDistance : fillPrice - exchangeSlDistance) : null,
            ExchangeTakeProfitPrice = config.TpSl.Enabled && !config.Exits.AtrTrailingRegimeEnabled ? (side == "SHORT" ? fillPrice - exchangeTpDistance : fillPrice + exchangeTpDistance) : null,
